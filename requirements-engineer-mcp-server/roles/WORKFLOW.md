...
1. **IT Lead Agent** calls `decompose_requirements` tool to break down high-level requirements
2. **Requirements Engineer Agent** receives decomposed requirements and formalizes them
3. **Requirements Engineer Agent** identifies ambiguities and may escalate to **Human Stakeholders** using `escalate_to_human`
4. **Requirements Engineer Agent** creates structured requirements specification (SRS)
5. **IT Lead Agent** validates requirements using quality gates
...
- **Information Exchange**: Requirements documents, project context, stakeholder inputs
- **Frequency**: At project initiation and when requirements change
- **Protocol**: Standard MCP request-response pattern
