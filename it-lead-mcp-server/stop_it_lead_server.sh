...
# Script to stop the IT Lead MCP Server
# Usage: ./stop_it_lead_server.sh [options]

echo "Stopping IT Lead MCP Server..."
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    -h|--help)
      ...
      echo ""
      echo "Options:"
      echo "  --port PORT               Port of the server to stop [default: 3061]"
...
if command -v lsof >/dev/null 2>&1; then
  ...
  echo "Attempting to kill python processes that might be IT Lead MCP servers..."
  pkill -f "it_lead_mcp_server.server" 2>/dev/null || true
  sleep 2
  pkill -9 -f "it_lead_mcp_server.server" 2>/dev/null || true
fi

echo "IT Lead MCP Server on port $PORT should be stopped."