"""
Extended IT Lead Server Handlers for MCP Server
Implements enhanced IT lead specific functionality for software development teams
...
from .advanced_assignment_handlers import AdvancedAssignmentHandlers
...
class ExtendedItLeadServerHandlers:
    """Handles extended IT lead specific MCP server methods for software development teams"""
    ...
    def __init__(self, enable_registry: bool = True, use_postgres: bool = True,
        ...
        self.enhanced_agent_registry = EnhancedAgentRegistry(self.service_registry)

        # Initialize enhanced handler modules
        self.strategic_planning_handlers = StrategicPlanningHandlers(
            llm_client=self.llm_client,
            agent_registry=self.enhanced_agent_registry,
            task_storage=self.task_storage
        )
        
        self.advanced_assignment_handlers = AdvancedAssignmentHandlers(
            llm_client=self.llm_client,
            agent_registry=self.enhanced_agent_registry,
            task_storage=self.task_storage
        )
        
        self.quality_gate_handlers = QualityGateHandlers(
            llm_client=self.llm_client,
            agent_registry=self.enhanced_agent_registry,
            task_storage=self.task_storage
        )
        
        self.human_interface_handlers = HumanInterfaceHandlers(
            llm_client=self.llm_client,
            agent_registry=self.enhanced_agent_registry,
            task_storage=self.task_storage
        )
        
        self.advanced_orchestration_handlers = AdvancedOrchestrationHandlers(
            llm_client=self.llm_client,
            agent_registry=self.enhanced_agent_registry,
            task_storage=self.task_storage
        )
    ...
    def _add_enhanced_resources(self):
        ...
        # Add all enhanced resources to the main resources list
        self.resources.extend(enhanced_resources)
    ...
    def _add_registry_methods(self):
        ...
        # Only add registry tools if the registry is actually available and initialized
    ...
    def register_handlers(self, rpc_handler: JsonRpcHandler):
        ...
        # Registry handlers - available when registry is enabled and properly initialized
        if self.enable_registry and self.service_registry is not None:
        ...
        # Register enhanced module handlers
        self.strategic_planning_handlers.register_handlers(rpc_handler)
        self.advanced_assignment_handlers.register_handlers(rpc_handler)
        self.quality_gate_handlers.register_handlers(rpc_handler)
        self.human_interface_handlers.register_handlers(rpc_handler)
        self.advanced_orchestration_handlers.register_handlers(rpc_handler)
    ...
    def _read_resource(self, resource: Dict[str, Any]) -> Dict[str, Any]:
        ...
        elif uri == "it-lead://resource/strategic-plan":
            # Return strategic plan resource
            return {
                "contents": [{
                    "uri": uri,
                    "text": json.dumps({
                        "plan_id": f"strategic-plan-{int(time.time())}",
        ...
        elif uri == "it-lead://resource/quality-dashboard":
            # Return quality dashboard resource
            return {
                "contents": [{
                    "uri": uri,
                    "text": json.dumps({
                        "dashboard_id": f"quality-dashboard-{int(time.time())}",
                        "created_at": time.time(),
                        "metrics": {
                            "overall_quality_score": 0.85,
                            "pass_rate": 0.92,
                            "average_defect_density": 0.05,
                            "test_coverage": 0.88
                        },
                        "trends": {}
                    }, indent=2)
        ...
        elif uri == "it-lead://resource/progress-report":
            # Return progress report resource
            return {
                "contents": [{
                    "uri": uri,
                    "text": json.dumps({
                        "report_id": f"progress-report-{int(time.time())}",
                        "created_at": time.time(),
