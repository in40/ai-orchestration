...
class JsonRpcHandler:
    ...
    def _handle_notification_sync(self, message: JsonRpcMessage):
        ...
        if method in self.notification_handlers:
            ...
            # FIX: Check if handler is async and run in event loop
            if asyncio.iscoroutinefunction(handler):
                try:
                    # Try to get existing event loop
                    loop = asyncio.get_event_loop()

                    # Check if loop is already running (nested async call)
                    if loop.is_running():
                        # Can't run a new loop while one is running.
                        # Schedule the coroutine as a task on the running loop.
                        # This is acceptable for notifications which don't need sync results.
                        asyncio.run_coroutine_threadsafe(handler(message.params), loop)
                    else:
                        # Loop exists but not running, use it
                        loop.run_until_complete(handler(message.params))
