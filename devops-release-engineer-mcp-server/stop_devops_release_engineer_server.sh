...
# Script to stop the DevOps Release Engineer MCP Server
# Usage: ./stop_devops_release_engineer_server.sh [options]

echo "Stopping DevOps Release Engineer MCP Server..."
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    -h|--help)
      ...
      echo ""
      echo "Options:"
      echo "  --port PORT               Port of the server to stop [default: 3071]"
...
if command -v lsof >/dev/null 2>&1; then
  ...
  echo "Attempting to kill python processes that might be DevOps Release Engineer MCP servers..."
  pkill -f "devops_release_engineer_mcp_server.server" 2>/dev/null || true
  sleep 2
  pkill -9 -f "devops_release_engineer_mcp_server.server" 2>/dev/null || true
fi

echo "DevOps Release Engineer MCP Server on port $PORT should be stopped."
