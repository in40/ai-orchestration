...
# 2. Team Management Server (port 3063)
# 3. IT Lead Server (port 3061)
# 4. Requirements Engineer (port 3062)
# 5. Implementation Engineer (port 3060)
# 6. Registry Server (mcp-std-skeleton) - port 3031
...
cleanup() {
    ...
    if [ -n "$WEBUI_PID" ]; then
        echo "Step 1/6: Stopping Web UI (PID $WEBUI_PID)..."
    ...
    if [ -n "$TMGT_PID" ]; then
        echo "Step 2/6: Stopping Team Management Server (PID $TMGT_PID)..."
    ...
    if [ -n "$IT_LEAD_PID" ]; then
        echo "Step 3/6: Stopping IT Lead Server (PID $IT_LEAD_PID)..."
    ...
    if [ -n "$REQ_ENG_PID" ]; then
        echo "Step 4/6: Stopping Requirements Engineer (PID $REQ_ENG_PID)..."
    ...
    if [ -n "$IMPL_PID" ]; then
        echo "Step 5/6: Stopping Implementation Engineer (PID $IMPL_PID)..."
    ...
    if [ -n "$REG_PID" ]; then
        echo "Step 6/6: Stopping Registry Server (PID $REG_PID)..."
...
RUNNING=$(pgrep -c -f "(mcp_std_server|it_lead_mcp_server|team_management)" 2>/dev/null || echo "0")
