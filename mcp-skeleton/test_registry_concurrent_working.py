...
class RegistryBasedConcurrentTester:
    ...
    async def get_available_services(self) -> List[Dict[str, Any]]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            # Start an SSE listener to receive the response
            response_received = asyncio.Event()
            registry_response = {}
            
            async def listen_for_registry_response():
                        ...
                        async for line in response.content:
                            ...
                            if line_str.startswith('data: '):
                                try:
                                    ...
                                    # Check if this is a response to our registry query
                                    req_id = data.get('id')
                                    if req_id and req_id.startswith('registry-query-'):
                                        ...
                                        break  # We got our registry response
                                except json.JSONDecodeError:
                                    continue
                            elif line_str.startswith(': ping'):
                                # Ping message, ignore
            ...
            sse_task = asyncio.create_task(listen_for_registry_response())
            
            # Send registry/list request (using correct method name from working script)
            request_id = f"registry-query-{uuid.uuid4()}"
            payload = {
                "jsonrpc": "2.0",
                "id": request_id,
                "method": "registry/list",  # Correct method name from working script
                "params": {}
            }
    ...
    async def run_registry_based_concurrent_test(self, num_requests_per_service: int) -> List[Dict[str, Any]]:
        ...
        for service in services:
            ...
            successful_count = sum(1 for r in service_results if r['success'])
            print(f"  Results: {successful_count}/{len(service_results)} successful")
