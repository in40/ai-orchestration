#!/usr/bin/env python3
"""
Main entry point for the DNS Resolver MCP server.

This script initializes and runs a DNS resolver MCP server that provides
DNS resolution services through the Model Context Protocol.
"""
...
from .server import DNSResolverMCPServer


def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description="DNS Resolver MCP Server")
    parser.add_argument(
        "--transport",
        choices=["stdio", "http"],
...
async def main():
    ...
    logger.info(f"Starting DNS resolver MCP server with transport: {config.transport}")

    # Initialize the DNS resolver server
    server = DNSResolverMCPServer(
