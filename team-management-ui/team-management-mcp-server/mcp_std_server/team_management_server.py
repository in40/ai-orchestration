...
Implements a specialized MCP server for team management functionality
"""
import signal
import sys
import threading
...
from .handlers.team_management_handlers import TeamManagementServerHandlers
...
class TeamManagementMcpServer:
    """Team Management MCP Server implementation that combines all components"""

    def __init__(self, transport_type: str = "streamable-http", host: str = "127.0.0.1", port: int = 3063, enable_registry: bool = False,
        ...
        self.host = host
        self.port = port  # Default to 3063 as requested
        self.running = False
        ...
        # Use the team management handlers instead of the base handlers
        self.server_handlers = TeamManagementServerHandlers(
        ...
        if self.enable_registry:
            ...
            self.service_info = {
                "id": f"team-management-registry-{host}:{port}",
                "name": "Team Management Service Registry",
                "description": "Central registry for team management services",
    ...
    def _register_with_registry(self):
        ...
        try:
            ...
            self.service_info = {
                "id": f"team-management-server-{self.host}-{self.port}",
                "name": f"Team Management MCP Server on {self.host}:{self.port}",
                "description": f"Team management server providing team management services on {self.host}:{self.port}",
    ...
    def start(self):
        ...
        print(f"Starting Team Management MCP server with {self.transport_type} transport...")
        self.running = True

        # Start the transport
        self.transport.start(self._message_callback)

        print(f"Team Management MCP server running with {self.transport_type} transport on port {self.port}")
    ...
    def stop(self):
        """Stop the MCP server"""
        print("Stopping Team Management MCP server...")
        ...
        self.running = False
        self.transport.stop()
        print("Team Management MCP server stopped")


def main():
    """Main entry point for the Team Management MCP server"""
    parser = argparse.ArgumentParser(description='Team Management MCP (Model Context Protocol) Server')
    ...
    parser.add_argument('--port',
                       type=int,
                       default=3063,  # Changed default to 3063 as requested
                       help='Port for HTTP transport (default: 3063)')
    ...
    server = TeamManagementMcpServer(
