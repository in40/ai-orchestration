...
class McpServer:
    ...
    async def _async_message_callback(self, message):
        """Async version of message callback for HTTP transport"""
        try:
            # Use the async version of handle_message for HTTP transport
            response = await self.rpc_handler.handle_message(message)

            if response:
                await self._async_send_response(response)
        except Exception as e:
            # Log error and send error response if it was a request
            if hasattr(message, 'message_type') and message.message_type.value == 'request':
                error_response = self.rpc_handler._create_error_response(
                ...
                await self._async_send_response(error_response)
            else:
                # For notifications, just log the error
                self.transport.send_error(f"Error handling message: {e}")

    async def _async_send_response(self, response):
        """Async version of send response for HTTP transport"""
        # Use the transport's specific response method if available
        if hasattr(self.transport, '_send_response'):
            if asyncio.iscoroutinefunction(self.transport._send_response):
                await self.transport._send_response(response)
            else:
                # For sync response method, we need to handle it differently
                # Instead of using run_in_executor, we can call it directly since it handles async operations internally
                self.transport._send_response(response)
        else:
            if asyncio.iscoroutinefunction(self.transport.send_message):
                await self.transport.send_message(response)
            else:
                # Run the sync method directly
    ...
    def start(self):
        ...
        # Choose the appropriate message callback based on transport type
        if self.transport_type == "http":
            # For HTTP transport, use the async callback
            message_callback = self._async_message_callback
        else:
            # For stdio transport, use the sync callback
            message_callback = self._message_callback

        # Start the transport
        self.transport.start(message_callback)
...
def main():
    ...
    # Import the AI Coding Agent server
    import sys
    import os
    sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    from ai_coding_agent_server import AiCodingAgentServer
