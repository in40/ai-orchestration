...
# Enhanced MCP Server Startup Script with Background Options
...
usage() {
    ...
    echo "  -l, --log-file FILE     Log output to specified file (implies background)"
    ...
    echo "  $0 -b -l server.log                  # Start in background with logging"
    echo "  $0 -R -b                             # Start and register in background"
    echo "  $0 -r --use-postgres -b              # Start registry with PostgreSQL in background"
    exit 1
}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -l|--log-file)
            LOG_FILE="$2"
            BACKGROUND=true  # Log file implies background
            shift 2
            ;;
        --pid-file)
            PID_FILE="$2"
...
if [[ "$BACKGROUND" == true ]]; then
    ...
    if [[ -n "$LOG_FILE" ]]; then
        ...
        echo "Starting MCP Server in background (output redirected to /dev/null)..."
        $CMD > /dev/null 2>&1 &
        SERVER_PID=$!
    fi
    ...
    echo "MCP Server started in background with PID: $SERVER_PID"
    echo "Command: $CMD"
else
    if [[ -n "$LOG_FILE" ]]; then
